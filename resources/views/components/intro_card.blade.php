...
<div {{$attributes->merge(["class" => "border-2 border-black rounded-lg flex flex-col no-underline hover:-translate-y-1 hover:shadow-[5px_5px_0_#111] transition-all duration-200"])}}>
    <div class="{{ $bg_color }} flex-1 p-5 flex flex-col gap-3 rounded-t-lg">
        <p class="font-header text-lg text-black">{{ $title }}</p>
        <p class="text-xs md:text-sm font-main text-gray-600">{{ $slot }}</p>
    </div>
    <a href="{{ route($route) }}">
